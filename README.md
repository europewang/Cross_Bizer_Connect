...
贝塞尔曲线线要素连接工具是一个专为ArcGIS Desktop设计的Python脚本工具，用于在线要素之间创建平滑的贝塞尔曲线连接。该工具通过智能的斜率分组算法，自动识别和连接相关的线要素，生成美观且符合几何原理的贝塞尔曲线。
...
#### 5. 配置脚本工具 - 参数
这是最关键的一步，您需要在这里定义脚本的输入和输出参数。请按照下面的说明添加四个参数：
...
5. 在工具对话框中：
   ...
   - **输出贝塞尔曲线**: 指定一个路径和名称来保存生成的贝塞尔曲线
   - **曲线饱满度**: 根据需要调整此值，值越大，曲线越弯曲
   - **曲线平滑度**: 根据需要调整此值，值越大，曲线越平滑
6. 点击 **OK**，脚本将执行并在您指定的位置创建包含贝塞尔曲线的新要素类

## 参数说明

| 参数 | 类型 | 范围 | 默认值 | 说明 |
|------|------|------|--------|------|
| 输入线要素 | Feature Set | - | - | 需要连接的线要素集合 |
| 输出贝塞尔曲线 | Feature Class | - | - | 生成的贝塞尔曲线输出路径 |
| 曲线饱满度 | Double | 0.1-1.0 | 0.5 | 控制曲线的弯曲程度，值越大越弯曲 |
| 曲线平滑度 | Long | 10-200 | 50 | 控制曲线的点密度，值越大越平滑 |
